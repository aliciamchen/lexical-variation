...
The `make test` target runs the data integrity suite on the processed CSVs together with unit tests for the derived-metric definitions (`analysis/test_compute_derived.py`) and for the preprocessing rules that the preregistration states precisely, such as the description-length flag and the treatment of trials with no referential message (`analysis/test_preprocessing.py`). It also runs `analysis/test_mixed_models.R`, which checks the random-effects simplification procedure that every mixed model goes through (`fit_progressively()` in `analysis/mixed_models.R`: the maximal structure, then correlations removed, then slopes dropped from the smallest variance component, as preregistered). The primary-analysis notebook computes Bayes factors for non-significant planned contrasts with `brms`, which takes several minutes per contrast; set the environment variable `BAYES_FACTORS=never` before rendering to skip them, or `BAYES_FACTORS=always` to compute them for every contrast.
