...
| `R/convergence.R` | `fit_h3b()` fits one linear-block model to group--tangram--block means over Phase 1 blocks 2--6 and returns both preregistered H3b contrasts, social-first minus social-mixed, neither privileged: the rate, as a slope contrast (`h3b_slope_contrast()`, in `contrast`/`summary`/`supported`), and the level at block 3, as an `emmeans` contrast taken after the fit (`h3b_block_contrast()`, in `block_contrast`/`block_summary`/`block_supported`). Block is raw, not centered: the block-3 quantity comes from `emmeans` at `block = H3B_BLOCK`, and centering only moves the uninterpreted `condition` coefficient. There is no game term, because in Phase 1 the three groups of a game never interact. |
