...
1. **Sync inputs.** If figures changed, run `bash figures/sync_figures.sh` (copies `SI_*.pdf` into `writing/preregistration/figures/`). If notebooks were re-rendered, confirm `writing/preregistration/stats/pilot.tex` and `writing/preregistration/stats/llm.tex` carry the new values; they have a `% AUTO-GENERATED` header and must not be hand-edited. Check that every macro used in `main.tex` is defined:
