...
H3B_FORMULA <- similarity ~ condition * block +
  (block | gameId) + (block | group) +
  (condition * block | target) + (1 | group_target)

H3B_TREND_FORMULA <- similarity ~ condition * blockNum_c +
  (blockNum_c | gameId) + (blockNum_c | group) +
  (condition * blockNum_c | target) + (1 | group_target)
...
h3b_block_contrast <- function(model) {
  em <- emmeans::emmeans(
    model, "condition", by = "block", at = list(block = as.character(H3B_BLOCK)),
  ...
  emmeans::contrast(
    em,
    list(H3b_block3_socialfirst_gt_socialmixed = pairwise_weights(
      levs, "social_first", "social_mixed"
    )),
    adjust = "none"
  )
}
...
h3b_supported <- function(contrast_summary) {
  nrow(contrast_summary) == 1L &&
    ...
    contrast_summary$estimate > 0 && contrast_summary$p.value < 0.05
}

# `block_means` comes from within_group_block_means(). Require replication
# at the game level in both conditions at the actual primary milestone.
# Retain the full trajectory for descriptive plots even if inference is skipped.
fit_h3b <- function(block_means, verbose = TRUE) {
  empty <- function(d, note) {
    list(data = d, model = NULL, contrast = NULL, summary = NULL,
         supported = NA, note = note)
  }
  if (!has_rows(block_means)) {
    return(empty(tibble(), "no within-group block means"))
  }
  d <- block_means |>
    filter(condition %in% SOCIAL_CONDITIONS, blockNum %in% 2:6,
           is.finite(similarity)) |>
    mutate(
      condition = factor(as.character(condition), levels = SOCIAL_CONDITIONS),
      block = factor(blockNum, levels = 2:6),
      blockNum_c = blockNum - H3B_BLOCK
    )
  at_milestone <- d |>
    filter(blockNum == H3B_BLOCK) |>
    distinct(condition, gameId)
  if (any(table(at_milestone$condition) < 2L)) {
    return(empty(d, "fewer than two games per condition with block-3 data"))
  }
  ...
  sm <- as.data.frame(summary(ct, infer = c(TRUE, TRUE), level = 0.95,
                              adjust = "none"))
  list(data = d, model = model, contrast = ct, summary = sm,
       supported = h3b_supported(sm), note = NULL)
}
